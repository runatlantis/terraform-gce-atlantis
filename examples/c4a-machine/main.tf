...
module "atlantis" {
  ...
  # Leave disk type inputs unset to allow the module to auto-select Hyperdisk for C4A machine types.
  # Uncomment the lines below if you need to force a different Hyperdisk flavor.
  # persistent_disk_type = "hyperdisk-extreme"
  # boot_disk_type       = "hyperdisk-extreme"
  domain  = local.domain
  project = local.project_id
}
